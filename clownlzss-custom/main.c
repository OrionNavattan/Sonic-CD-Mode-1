...
static void PrintUsage(void)
{
	fputs(
		...
		"This version has been modified to use a custom moduled header.\n"
		"\n"
					...
					else
					{
						...
						switch (mode->format)
						{
							...
							case FORMAT_KOSINSKI:
								if (moduled)
									success = ClownLZSS_ModuledCompressionWrapper(file_buffer, file_size, &callbacks, ClownLZSS_KosinskiCompress, module_size, 1);
								else
